...
async def validate_proxies_serial(
    ...
    Safety comes from *serial* execution (never more than one in-flight TCP
    ...
    async with AsyncSession() as session:
        for i, p in enumerate(proxies):
            if cancel_event is not None and cancel_event.is_set():
                clog(f"{Colors.YELLOW}[PROXY-SAFE] cancelled at proxy {i}/{total}{Colors.RESET}")
                break
            ok = False
            try:
                r = await session.get(
            ...
            # Check cancel after potentially slow HTTP request.
            if cancel_event is not None and cancel_event.is_set():
                clog(f"{Colors.YELLOW}[PROXY-SAFE] cancelled after HTTP (proxy {i}/{total}){Colors.RESET}")
                break
            ...
            # Pause between checks (skip after the last one; skip if cancelled).
            if i < total - 1:
...
async def _do_setpr(update: Update, context: ContextTypes.DEFAULT_TYPE, add: bool) -> None:
    ...
    # Serial validation (one at a time, tiny jitter) is safe at any batch size
    # because there is never more than one in-flight TCP connection.
    ...
    try:
        await status.edit_text(
            f"Validating {len(candidates)} proxies *serially* (one at a time) ...\n"
            f"(zero burst risk — use /stop to cancel and keep proxies validated so far)",
            parse_mode=ParseMode.MARKDOWN,
        )
...
async def cmd_check(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pool_n > 0:
        status_msg = await msg.reply_text(
            f"Re-verifying {pool_n} proxies serially ...",
            parse_mode=ParseMode.MARKDOWN,
        )
